...
def _amministrazione_dashboard_context():
...
def home(request):
    ...
    if request.user.is_authenticated and request.user.is_amministratore_app:
        return render(
            request,
            'mensa/amministrazione/dashboard.html',
            _amministrazione_dashboard_context(),
        )
...
def catalogo_lotti(request):
    ...
    if request.user.is_studente or request.user.is_amministratore_app:
        return redirect('home')
...
def chiudi_lotto(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'Lotto eliminato dai lotti pubblicati.')
...
class ProdottoList(BaseList):
    ...
    fields = ['nome', 'categoria', 'vegetariano', 'vegano']
